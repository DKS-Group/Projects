...
class Topcomm_Widget extends WP_Widget {
  ...
  // start function widget 
	function widget( $args, $instance ) {
		...
		if($listType == "num") {
			$listStart = "<ol class='top_comm_list'>";
			$listEnd = "</ol>";
		} else {
			$listStart = "<ul class='top_comm_list'>";
			$listEnd = "</ul>";
		} // end list type setup
    ...
    if(count($commenters) > 0) {
      ...
      foreach ($commenters as $k) {
        ...
        if(strlen($k->comment_author) > $limitChar) {
          $str = "<span>". substr($k->comment_author, 0, $limitChar-4) . "...</span>";
        } else {
          $str = "<span>".$k->comment_author . "</span>";
        }
        $writeList .= $str;
        if($showCount == 1)
          $writeList .= ' <span class="numb_comm_text">' . $nCommentComments . '</span>';
        if(trim($url) != '') {
          if($makeLink == 1)
            $writeList .= "</a>";
        }
